...
export const schedulersDueTomorrow = computed(
  () => usePrivateSchedulerState().schedulersDueTomorrow
);
...
export const activeSchedulersThisWeek = computed(() => {
  ...
  const isActiveScheduler = (scheduler: Scheduler) => {
    ...
    const activeOrder = account.attributes.orders?.data.some(
      order => order.attributes.isActive && !order.attributes.OrderPaid
    );
