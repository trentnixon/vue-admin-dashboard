...
export const totalDownloadCount = computed(
  () => usePrivateDownloadState().totalDownloadCount
); // Add this line
...
export const downloadsInLast24Hours = computed(() => {
  ...
  return downloads.value.filter(download => {
    const completedDate = download.attributes.updatedAt
      ? new Date(download.attributes.updatedAt)
      : null;
    return (
      completedDate &&
      isWithinInterval(completedDate, { start: yesterday, end: now })
    );
  }).length;
});
